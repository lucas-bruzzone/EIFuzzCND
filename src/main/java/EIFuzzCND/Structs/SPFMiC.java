...
public class SPFMiC implements Instance {
    private double Me; //soma linear das pertinências elevadas a m
    private double Te; //soma linear das tipicidades elevadas a n
    ...
    private double N; // numero de exemplos presentes no micro-grupo
    private double t;
    ...
    public void atribuiExemplo(Example exemplo, double pertinencia, double tipicidade, double m, double n) {
        ...
        this.N++;
        this.Me += Math.pow(pertinencia, m);
        this.Te += Math.pow(tipicidade, n);
    ...
    public double calculaPertinencia(double[] exemplo, double n, double K) {
        double pertinencia = this.getPertinencia(K);
        double dist = DistanceMeasures.calculaDistanciaEuclidiana(exemplo, this.centroide);
        return (1 /
                (1 + Math.pow(((this.theta/ pertinencia) * dist),
                        (1/(n-1))
                )
                ));
    }

    private double getPertinencia(double K) {
        return  K * (this.SSDe / this.Me);
    }
